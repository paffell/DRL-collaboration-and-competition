...
UPDATE_EVERY = 10  # how often to train the agent
LEARN_TIMES = 5  # how many times to train the agent in a row
...
class Agent():
    ...
    def act(self, state, eps):
        ...
        if random.random() > eps:
            ...
            return np.array([random.choice(np.arange(self.action_size)),random.choice(np.arange(self.action_size))])
...
#         Q_targets = rewards + (gamma * Q_targets_next * (1 - dones))
        Q_targets = rewards + (gamma * Q_targets_next * (1 - 0)) ###########################
        # Compute critic loss
